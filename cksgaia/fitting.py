...
efudge = np.array([ 2.81570765,  2.49756761,  2.30114599,  2.5389933 ,  2.34539564,
        ...
        1.56983652,  1.36153125,  1.34642358,  1.44756085,  1.4414092 ])
...
#         prad_noise = np.random.normal(0, pm['iso_prad_err'].values, size=len(pm))
#         pm['iso_prad'] += prad_noise
#
#         pm = cksrad.completeness.get_weights(pm, kicselect)
#
#         wdetections,_,_ = np.histogram2d(pm['koi_period'], pm['iso_prad'], bins=[Pedges,Redges], weights=pm['weight'])
#
...
def histfit(physmerge, verbose=True, completeness=True, boot_errors=False):
    ...
    if completeness:
        ...
        detections,_,_ = np.histogram2d(physmerge['koi_period'], physmerge['iso_prad'], bins=[Pedges,Redges])
        wdetections,_,_ = np.histogram2d(physmerge['koi_period'], physmerge['iso_prad'], bins=[Pedges,Redges], weights=physmerge['weight'])
        ...
        N, bin_edges = np.histogram(physmerge['iso_prad'], bins=xbins)
